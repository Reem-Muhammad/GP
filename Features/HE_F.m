function [ HE_outputMatrix ] = HE_F( data )

[n_samples, n_channels] = size(data); %obtain the dimensions of the record
windowSize = 1024; %equivalent to 4 seconds

HE_outputMatrix = zeros(n_channels, floor(n_samples/windowSize)); %initialize the output matrix %--------<

disp('Feature:');
disp('Hurst Exponent');

%%%%%HE_outputMatrix = []; %stores the HE values of all channels. each row has the HE values of the windows of a single channel
...
for ch=1:n_channels
    ...
    %%%%%HE_vector_window = []; %initialize the vector before processing each channel
    ...
    [window_Nrow, window_Ncol] = size(windows);
        ...
        for window_iter = 1:window_Ncol
            ...
            he = HE(ProcessedWindow');
            %%%HE_vector_window = [HE_vector_window he]
            HE_outputMatrix(ch, window_iter)= he;
        end
      %%%%%%%%%%HE_outputMatrix = [HE_outputMatrix; HE_vector_window];  
end
